var jsbreadcrumbs_controller = ( function () {
	...
	return {
		...
		initialize: function () {
					...
					if ( visibleCrumbs.length < siteMaxCrumbs ) {
						...
						if ( !( 'action' in breadcrumb ) || showAction ||
							breadcrumb.action.length === 0 ) {
								...
								visibleCrumbs.push( a.outerHTML );
						} else {
							breadcrumbs.splice( index, 1 );
						}
					} else {
			...
			// save the breadcrumbs to the cookie, sanitize first
			function sanitize(str) {
				return typeof str === 'string'
					? str.replace(/<[^>]*>/g, '') // removes full HTML tags
					: str;
			}

			const safeBreadcrumbs = breadcrumbs.map(b => ({
				url: sanitize(b.url),
				title: sanitize(b.title),
				action: sanitize(b.action),
				siteName: sanitize(b.siteName)
			}));

			// For Debugging
			// console.log('Try 1: Sanitized breadcrumbs:', safeBreadcrumbs);

			mw.cookie.set(
				'mwext-jsbreadcrumbs',
				JSON.stringify( safeBreadcrumbs ),
			...
			if ( horizontal ) {
				...
				if ( skin === 'vector' || skin === 'vector-2022' ) {
					...
					$( '#p-rv' ).append( $( '<h3>' ).attr( 'id', 'p-rv-label' ).text( leadingDescription ) );
